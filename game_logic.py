...
# Функція для вгадування слова
def guess_word(target_word, max_attempts=5):  # Додано параметр max_attempts
    attempts = 0
    guessed_word = ["_"] * len(target_word)

    layout = [
        [sg.Text(" ".join(guessed_word), key="word_display", size=(30, 1), justification="center")],
        [sg.Text(f"Залишилося спроб: {max_attempts - attempts}", key="attempts_display")],  # Відображення залишилися спроб
    ...
    while attempts < max_attempts:  # Змінено умову завершення гри
        ...
        if user_guess == target_word:
            ...
            window["attempts_display"].update(f"Залишилося спроб: {max_attempts - attempts}")  # Оновлення відображення спроб

    if attempts >= max_attempts:  # Додано умову для випадку вичерпання спроб
...
if __name__ == "__main__":
    ...
    guess_word(target_word)  # Виклик функції для вгадування слова
=======
import PySimpleGUI as sg
import random
